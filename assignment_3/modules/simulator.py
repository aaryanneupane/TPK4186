...
class Simulator:
    ...
    def execute_simulation_loop(self):
        ...
        print(
            f"At the end of the simulation of {self.time} seconds, with {len(warehouse.get_robots())} robots, these are our results:\n"
        )

        print(
            f"The total amount of orders delivered were: {number_of_completed_orders}\n"
        )

        print(
            f"The total amount of cancelled orders were: {number_of_cancelled_orders}\n"
        )

        print(
            f"The amount of remaining orders we could not deliver: {remaining_orders}\n"
        )

        print(
            f"The average amount of time it took to pick up the order was {average_pick_up_time}"
        )
